...
import {
  ...
  FocusSession, NotebookEntry, loadCustomTasks, saveCustomTasks,
  loadApiSettings, saveApiSettings, ApiSettings, loadTheme, saveTheme,
} from "./types";
...
import { Terminal, ShieldBan, Sparkles, Sun, Moon, Settings, Key, X, ChevronDown, Bot } from "lucide-react";
...
function loadRecords(): DailyRecord[] {
  try { const raw = localStorage.getItem("kaoyan_records_v2"); return raw ? JSON.parse(raw) : []; }
  catch { return []; }
}
function saveRecords(records: DailyRecord[]) { localStorage.setItem("kaoyan_records_v2", JSON.stringify(records)); }

export default function App() {
  const [tasks, setTasks] = useState<TaskDef[]>(() => loadCustomTasks());
  ...
  const [notebookEntries, setNotebookEntries] = useState<NotebookEntry[]>(() => {
    try { const raw = localStorage.getItem("kaoyan_notebook_entries"); return raw ? JSON.parse(raw) : []; }
    catch { return []; }
  });
  ...
  function saveNotebookEntries(entries: NotebookEntry[]) {
    localStorage.setItem("kaoyan_notebook_entries", JSON.stringify(entries));
  }
  ...
  useEffect(() => {
    ...
    if (showTimer) { setLuluState("peaceful"); return; }
    setLuluState("peaceful");
  }, [showChat, tasks, showTimer]);
  ...
  const handleTasksChange = (ts: TaskDef[]) => { setTasks(ts); saveCustomTasks(ts); };

  const handleTasksFromLulu = (ts: TaskDef[]) => {
    setTasks(ts); saveCustomTasks(ts); setActiveTaskId(null); setLuluState("happy");
    const d = todayStr();
    setRecords((p) => { const nr = [{ date: d, tasks: ts.map((t) => ({ taskId: t.id, completed: false, sessions: [] })) }, ...p.filter((r) => r.date !== d)]; saveRecords(nr.slice(0, 365)); return nr.slice(0, 365); });
  };
  ...
  const handleAddNotebookEntry = (entry: NotebookEntry) => {
    setNotebookEntries((prev) => { const next = [entry, ...prev]; saveNotebookEntries(next); return next; });
  };

  const handleDeleteNotebookEntry = (id: string) => {
    setNotebookEntries((prev) => { const next = prev.filter((item) => item.id !== id); saveNotebookEntries(next); return next; });
  };

  const persistSession = useCallback((taskId: string, session: FocusSession) => {
    const d = todayStr();
    setRecords((p) => {
      const ex = p.find((r) => r.date === d);
      const nr = ex ? p.map((r) => r.date !== d ? r : { ...r, tasks: (() => { const tr = r.tasks.find((s) => s.taskId === taskId); return tr ? r.tasks.map((s) => s.taskId === taskId ? { ...s, sessions: [...s.sessions, session] } : s) : [...r.tasks, { taskId, completed: false, sessions: [session] }]; })() }) : [{ date: d, tasks: tasks.map((t) => ({ taskId: t.id, completed: false, sessions: t.id === taskId ? [session] : [] })) }, ...p];
      saveRecords(nr.slice(0, 365)); return nr.slice(0, 365);
    });
  }, [tasks]);
  ...
  const markComplete = useCallback((taskId: string) => {
    ...
    setRecords((p) => {
      const ex = p.find((r) => r.date === d);
      const nr = ex ? p.map((r) => r.date !== d ? r : { ...r, tasks: r.tasks.map((s) => s.taskId === taskId ? { ...s, completed: true } : s) }) : [{ date: d, tasks: tasks.map((t) => ({ taskId: t.id, completed: t.id === taskId, sessions: [] })) }, ...p];
      saveRecords(nr.slice(0, 365)); return nr.slice(0, 365);
    });
  }, [tasks]);

  const handleTaskComplete = (id: string) => { markComplete(id); setTimeout(() => setActiveTaskId(null), 1500); };

  const handleFocusSessionEnd = (elapsedSeconds: number) => {
    if (activeTaskId) {
      const now = Date.now();
      persistSession(activeTaskId, { startTime: now - elapsedSeconds * 1000, endTime: now, durationMinutes: Math.round((elapsedSeconds / 60) * 10) / 10 });
    }
    setShowTimer(false);
  };
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden text-sm selection:bg-emerald-500/30">
      <div className="w-80 border-r border-zinc-800 bg-zinc-950/40 flex flex-col backdrop-blur-xl shrink-0">
        ...
        <Sidebar tasks={tasks} onSelectTask={(id) => { setActiveTaskId(id); setView("main"); }} activeTaskId={activeTaskId} onOpenStats={() => setView("stats")} onTasksChange={handleTasksChange} onOpenApiSettings={() => setShowApiSettings(true)} onOpenNotebook={handleOpenNotebook} onOpenChat={() => setShowChat(true)} luluState={luluState} />
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-zinc-900/20 via-zinc-950 to-zinc-950">
        <header className="h-16 border-b border-zinc-800/60 bg-zinc-950/80 flex items-center justify-between px-8 z-10 backdrop-blur-md shrink-0">
          ...
          <div className="flex items-center gap-3">
            {activeTask && view === "main" && !showTimer && <button onClick={() => setShowTimer(true)} className="text-xs font-mono text-emerald-400 border border-emerald-800 px-3 py-1.5 rounded hover:bg-emerald-950/30 transition-colors">开始计时</button>}
            <Lulu state={luluState} onClick={() => setShowChat(true)} />
          </div>
        </header>
        <main className="flex-1 overflow-y-auto p-10">
          {view === "stats" ? <StatsPanel records={records} tasks={tasks} onBack={() => setView("main")} onStartTask={(id: string) => { setActiveTaskId(id); setView("main"); setShowTimer(true); }} onDeleteRecord={(date) => { setRecords((prev) => { const next = prev.filter(r => r.date !== date); saveRecords(next.slice(0, 365)); return next.slice(0, 365); }); }} onDeleteTaskFromRecord={(date, taskId) => { setRecords((prev) => { const next = prev.map(r => r.date === date ? { ...r, tasks: r.tasks.filter(t => t.taskId !== taskId) } : r).filter(r => r.tasks.length > 0); saveRecords(next.slice(0, 365)); return next.slice(0, 365); }); }} /> : !activeTask ? (
            ...
            <div className="max-w-5xl mx-auto h-full flex flex-col">
              <div className="mb-10"><div className="flex items-center gap-3 mb-2"><span className="text-[10px] border border-zinc-700 text-zinc-400 px-2 py-0.5 rounded font-mono uppercase bg-zinc-800">预计 {activeTask.estimatedMinutes} 分钟</span>{activeTask.status === "completed" && <span className="text-[10px] bg-emerald-950/50 border border-emerald-900 text-emerald-400 font-mono px-2 py-0.5 rounded">VERIFIED</span>}</div><h2 className="text-3xl font-bold text-zinc-100 mb-1 tracking-tight">{activeTask.title}</h2><p className="text-zinc-400 text-sm">{activeTask.description}</p></div>
              {activeTask.status === "completed" ? (() => {
            const today = todayStr();
            const todayRecord = records.find(r => r.date === today);
            const taskSessions = todayRecord?.tasks.find(t => t.taskId === activeTask.id)?.sessions || [];
            const lastSession = taskSessions.length > 0 ? taskSessions[taskSessions.length - 1] : null;
            return (
              <div className="border border-emerald-900/50 bg-emerald-950/20 p-8 rounded-lg flex flex-col items-center justify-center text-emerald-500 font-mono">
                ...
                {lastSession && lastSession.startTime && (
                  <div className="mt-3 text-xs text-zinc-500">
                    开始: {new Date(lastSession.startTime).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}
                  </div>
                )}
              </div>
            );
          })() : <Workspace task={activeTask} onComplete={() => handleTaskComplete(activeTask.id)} onStartTimer={() => setShowTimer(true)} timerActive={showTimer} />}
            </div>
          )}
        </main>
        {showTimer && activeTask && <FocusTimer taskName={activeTask.title} focusMinutes={activeTask.estimatedMinutes} onEnd={handleFocusSessionEnd} onClose={() => setShowTimer(false)} />}
      </div>
      {showNotebook && <NotebookModal entries={notebookEntries} onClose={handleCloseNotebook} onAddEntry={handleAddNotebookEntry} onDeleteEntry={handleDeleteNotebookEntry} />}
